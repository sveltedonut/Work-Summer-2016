...
wcoefs  = getcoef(Wfdobj);

wnbasis = getnbasis(wbasis);
wrange  = getbasisrange(wbasis);
wparam  = getbasispar(wbasis);
if isempty(wparam)
    wbreak   = wrange;
else
    wbreak   = [wrange(1), wparam, wrange(2)];
end
worder   = wnbasis - length(wbreak) + 2;
...
[iter, tin, Hold, DHold, D2Hold] = thetafunC(itermax, tin, wbreak, worder, wcoefs, Uin, N, Slambda, tQin, scrrng, crit);
