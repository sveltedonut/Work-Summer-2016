...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    // Macros for input
    #define RNGVEC      prhs[0]
    #define JMAX        prhs[1]
    #define JMIN        prhs[2]
    #define NREP1       prhs[3]
    #define NREP2       prhs[4]
    #define FDOBJ1      prhs[5]
    #define FDOBJ2      prhs[6]
    #define LFDOBJ1     prhs[7]
    #define LFDOBJ2     prhs[8]
    #define WTFD        prhs[9]
    #define EPS         prhs[10]
    ...
    int jmax, jmaxp, irng, nrng, nrep1, nrep2, tnm, iter, *D, i, j, k, m, index, ns, jmin;
                ...
                for (j = 0; j < nrep1; j++) {
                    for (k = 0; k < nrep2; k++) {
                        ...
                        for (i = 0; i < LAST_APPROX; i++) {
                            ds[i] = ya[i];
                            cs[i] = ya[i];
                            //mexPrintf("ds[%d] = ", i + LAST_APPROX - iter);
                            //mexPrintf("%f, ", ds[i + LAST_APPROX - iter]);
                            //mexPrintf("cs[%d] = ", i + LAST_APPROX - iter);
                            //mexPrintf("%f\n", cs[i + LAST_APPROX - iter]);
                        }
                        ...
                        for (m = 1; m < LAST_APPROX; m++) {
                            ...
                            //mexPrintf("lhs = %d, rhs = %d, ", 2*(ns + 1), LAST_APPROX - m);
