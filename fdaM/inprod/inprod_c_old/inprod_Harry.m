function  [inprodmat, iter] = ...
                     inprod_Harry(fdobj1, fdobj2, Lfdobj1, Lfdobj2, ...
...
%  Previous versions of INP ROD allowed the functions to be bivariate
...
nrng = length(rngvec);
for irng = 2:nrng
    %  Harray's work starts here!!
    
    %  rngi contains the initial and final values for the sub-range.
    rngi = [rngvec(irng-1),rngvec(irng)];
    %  change range so as to avoid being exactly on
    %  multiple knot values.  1e-10 could be problematic in some
    %  applications.
    if irng > 2
        rngi(1) = rngi(1) + 1e-10;
    end
    ...
    iter  = 1;
    width = rngi(2) - rngi(1);  %  width of interval of integration
    %  JMAX needed as argument
    JMAXP = JMAX + 1;
    h     = ones(JMAXP,1);
    h(2)  = 0.25;
    %  nrep1 and nrep2 needed as arguments
    ...
    %  objects fdobj1 and fdobj2 needed as struct arguments
    %  We know how to do this in C!
    %  Assume that function functions isa_fd and eval_fd have C versions
    if isa_fd(fdobj1)
...
%         end
    ...
    %  We may need fd object wtfd
    if ~isempty(wtfd)
        wtd = eval_fd(rngi, wtfd);
        fx2 = (wtd * ones(1,nrep2)) .* fx2;
    end
    
    % initialize the contents of s with inner product of initial points
    % the ith row of s contains the appproximations to the integral(s)
    % at iteration i.
    s(1,:,:)  = width.*(fx1' * fx2)./2;
        ssss = reshape(s(1,:,:), nrep1, nrep2);
    
    tnm = 0.5;  %  initialize half of number of trapezoids 
    
    %  now iterate to convergence
    %  at iteration, we double the number of trapezoids
    
    for iter = 2:JMAX
        tnm = tnm.*2;
        %  compute trapezoid width del by dividing range width by tnm
        del = width./tnm;
        x   = rngi(1)+del/2:del:rngi(2);  %  new points of evaluation
        if isa_fd(fdobj1)
...
%             end
        ...
        ret = reshape(s(iter,:,:), nrep1, nrep2);
        %  Richardson extrapolation step:  fit a polynomial to
        %  the current and last four approximations, extraolation to
        %  the next iteration, and quit if the difference between the
        %  current and extrapolated values is sufficiently small (EPS)
        if iter >= 5
            ind   = (iter-4):iter;
            ya    = s(ind,:,:);
            xa    = h(ind);
            ...
            y  = squeeze(ya(ns,:,:));
            ns = ns - 1;
            %  compute the extrapolated approximation update value ss  
            %  and error estimate dy recursively here
            for m = 1:4
                for i = 1:(5-m)
                    ho      = xa(i);
                    hp      = xa(i+m);
                ...
                %  compute current error estimate
                if 2*ns < 5-m
                    dy = squeeze(cs(ns+1,:,:));
                else
                    dy = squeeze(ds(ns,:,:));
        ...
        %  Here the extrapolation has failed, and we initialize another
        %  iteration.
        s(iter+1,:,:) = s(iter,:,:);
        h(iter+1)   = 0.25.*h(iter);
        %  if current iteration is JMAX, give up and warn the user that
        %  we have failed.
        if iter == JMAX
            warning('Wid:converge','Failure to converge.');
        end
    end
    %  update final returned approximation
    inprodmat = inprodmat + ss;
%     Harray's job ends here!!
end
